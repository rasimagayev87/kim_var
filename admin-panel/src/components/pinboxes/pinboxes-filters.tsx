...
export function PinBoxesFilters({ initialSearch, initialStatus }: { initialSearch: string; initialStatus: string }) {
  ...
  const updateParam = useCallback(
    (key: string, value: string | null) => {
      ...
      startTransition(() => {
        router.push(`/pinboxes?${params.toString()}`);
      });
    },
    [router, searchParams],
  );
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
      <Input
        placeholder="Qutu və ya məkan adı axtar..."
        value={search}
