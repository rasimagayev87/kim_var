...
class _SupportMessageSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_SupportMessageSheet> createState() =>
      _SupportMessageSheetState();
}
...
class _SupportMessageSheetState extends ConsumerState<_SupportMessageSheet> {
  ...
  Future<void> _send(AppLocalizations loc) async {
    ...
    final ok = await ref
        ...
        .sendMessage(widget.type, message);
    ...
    if (ok) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(loc.supportMessageSentNotice)));
    } else {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(loc.supportMessageErrorMessage)));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final title = widget.type == SupportMessageType.problem
        ? loc.reportProblemSheetTitle
        : loc.sendSuggestionSheetTitle;
