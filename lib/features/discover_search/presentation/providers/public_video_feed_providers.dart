...
class PublicVideoFeedController extends StateNotifier<PublicVideoFeedState> {
  PublicVideoFeedController(this._repository) : super(const PublicVideoFeedState()) {
    _subscription = _repository.watchPublicVideoFeed(limit: _kPageSize).listen(
      (page) {
        _firstPage = page;
        _recompute();
      },
      onError: (Object e, StackTrace st) {
        logError('public_video_feed_providers.PublicVideoFeedController', e, st);
  ...
  Future<void> loadMore() async {
    ...
    try {
      final more = await _repository.fetchMorePublicVideos(startAfter: oldest, limit: _kPageSize);
      _olderPages = [..._olderPages, ...more];
      state = state.copyWith(isLoadingMore: false, hasMore: more.length == _kPageSize);
      _recompute();
    } catch (e, st) {
      logError('public_video_feed_providers.PublicVideoFeedController.loadMore', e, st);
