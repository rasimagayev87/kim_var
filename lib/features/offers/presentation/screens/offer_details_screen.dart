...
class OfferDetailsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isFavorite = ref.watch(
      favoriteOfferIdsProvider.select(
        (async) => async.valueOrNull?.contains(offerId) ?? false,
      ),
    );
                ...
                SizedBox(
                  ...
                  child: OutlinedButton(
                    onPressed: () => Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (_) =>
                            VenueProfileScreen(venueId: offer.venueId),
                      ),
                    ),
                    ...
                    child: Text(
                      loc.offerViewVenueProfileButton,
...
class _HeroImage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final boostEnabled = ref.watch(
      featureFlagProvider(FeatureFlag.boostPayment),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final (color, text) = switch (offer.offerType) {
      ...
      OfferType.fixedPrice => (
        AppColors.cyanDark,
        '${offer.discountValue?.round() ?? 0} ${loc.offerBadgeFixedPriceSuffix}',
      ),
...
class _RedeemButtonState extends ConsumerState<_RedeemButton> {
  ...
  Widget build(BuildContext context) {
    final loc = AppLocalizations.of(context);
    final currentUid = fb.FirebaseAuth.instance.currentUser?.uid;
    if (currentUid == null || widget.offer.isOwnedBy(currentUid))
      return const SizedBox.shrink();

    final isRedeemed =
        ref.watch(isOfferRedeemedByMeProvider(widget.offer.id)).valueOrNull ??
        false;
    ...
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        label: Text(
          isRedeemed
              ? loc.offerRedeemedButtonLabel
              : loc.offerRedeemButtonLabel,
        ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            child: _ValidityDate(
              label: loc.offerStartDateLabel,
              date: dateFormat.format(offer.startDate),
            ),
          ),
          Container(
            width: 1,
            height: 28,
            color: ChatLightColors.inkFaint.withValues(alpha: 0.2),
          ),
          Expanded(
            child: _ValidityDate(
              label: loc.offerEndDateLabel,
              date: dateFormat.format(offer.endDate),
            ),
          ),
        ],
      ),
      ...
      child: SizedBox(
        ...
        child: IgnorePointer(
          child: GoogleMap(
            ...
            markers: {
              Marker(markerId: const MarkerId('offer'), position: center),
            },
