...
class VenueReviewsSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final reviews = ref.watch(venueReviewsProvider(venue.id)).valueOrNull ?? const <Review>[];
    final currentUid = fb.FirebaseAuth.instance.currentUser?.uid;
    final isOwner = currentUid != null && venue.isOwnedBy(currentUid);
    final verifiedEntry = isOwner ? null : ref.watch(verifiedVisitProvider(venue.id)).valueOrNull;
            ...
            children: [
              ...
              if (canWrite)
                TextButton(
                  ...
                  child: Text(loc.reviewsWriteButton, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.primary)),
                ),
            ],
          ),
...
class _RatingSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final average = reviews.fold<int>(0, (sum, r) => sum + r.rating) / reviews.length;
    final counts = List.generate(5, (i) => reviews.where((r) => r.rating == 5 - i).length);

    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: ChatLightColors.bg1, borderRadius: BorderRadius.circular(16)),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          Column(
            children: [
              Text(average.toStringAsFixed(1), style: const TextStyle(fontSize: 32, fontWeight: FontWeight.w800, color: ChatLightColors.ink)),
              ...
              children: [
                for (var i = 0; i < 5; i++)
                  Padding(
                    ...
                    child: _DistributionBar(star: 5 - i, count: counts[i], total: reviews.length),
                  ),
              ],
            ),
          ),
...
class _DistributionBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        SizedBox(width: 12, child: Text('$star', style: const TextStyle(fontSize: 10.5, color: ChatLightColors.inkSoft))),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            CircleAvatar(
              ...
              child: profile?.photoUrl == null ? const Icon(Icons.person_outline, color: ChatLightColors.inkSoft, size: 16) : null,
            ),
                ...
                children: [
                  Row(
                    children: [
                      Flexible(
                        child: Text(
                          profile?.name.trim().isNotEmpty == true ? profile!.name : (profile?.username ?? ''),
                  ...
                  Row(
                    children: [
                      VenueStarRating(rating: review.rating.toDouble(), size: 13),
                      const SizedBox(width: 6),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 1.5),
                        decoration: BoxDecoration(color: const Color(0xFFE3F8EC), borderRadius: BorderRadius.circular(20)),
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            const Icon(Icons.verified, size: 11, color: Color(0xFF1F9254)),
                            const SizedBox(width: 3),
                            Text(
                              loc.reviewsVerifiedBadge,
                              style: const TextStyle(fontSize: 9.5, fontWeight: FontWeight.w700, color: Color(0xFF1F9254)),
                            ),
                          ],
                        ),
                      ),
        ...
        Text(review.comment, style: const TextStyle(fontSize: 14, color: ChatLightColors.ink, height: 1.4)),
        if (review.ownerReply != null) ...[
          const SizedBox(height: 10),
          Container(
            margin: const EdgeInsets.only(left: 42),
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(color: ChatLightColors.bg1, borderRadius: BorderRadius.circular(12)),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(
                ...
                const SizedBox(height: 4),
                Text(review.ownerReply!.text, style: const TextStyle(fontSize: 13, color: ChatLightColors.ink, height: 1.35)),
              ],
            ),
          ),
          ...
          Align(
            ...
            child: TextButton(
              ...
              child: Text(loc.reviewsOwnerReplySubmitButton, style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: AppColors.primary)),
            ),
          ),
        ],
      ],
...
Future<void> _showOwnerReplySheet(BuildContext context, WidgetRef ref, Review review) async {
  final loc = AppLocalizations.of(context);
  ...
  final text = await showModalBottomSheet<String>(
    ...
    builder: (sheetContext) => Padding(
      padding: EdgeInsets.fromLTRB(20, 16, 20, 16 + MediaQuery.viewInsetsOf(sheetContext).bottom),
      child: SafeArea(
        top: false,
        child: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(loc.reviewsOwnerReplyLabel, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: ChatLightColors.ink)),
            const SizedBox(height: 12),
            TextField(
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text(loc.reviewsOwnerReplySubmitButton, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: AppColors.onAccent)),
              ),
            ),
          ],
        ),
  ...
  final ok = await ref.read(reviewControllerProvider).submitOwnerReply(reviewId: review.id, text: text);
  if (!context.mounted) return;
  if (!ok) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(loc.reviewsOwnerReplyErrorMessage)));
  }
}
...
class _ReportReviewButton extends ConsumerWidget {
  ...
  Future<void> _report(BuildContext context, WidgetRef ref) async {
      ...
      builder: (sheetContext) => SafeArea(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            ListTile(title: Text(loc.eventReportReasonSpam), onTap: () => Navigator.pop(sheetContext, 'spam')),
            ListTile(title: Text(loc.eventReportReasonInappropriate), onTap: () => Navigator.pop(sheetContext, 'inappropriate')),
            ListTile(title: Text(loc.eventReportReasonOther), onTap: () => Navigator.pop(sheetContext, 'other')),
            const SizedBox(height: 8),
          ],
        ),
      ),
    ...
    final ok = await ref.read(reviewControllerProvider).report(reviewId: review.id, venueId: review.venueId, reason: reason);
    if (!context.mounted) return;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return IconButton(
      ...
      icon: const Icon(Icons.flag_outlined, size: 16, color: ChatLightColors.inkFaint),
    );
  }
}
